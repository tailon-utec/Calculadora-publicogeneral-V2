...
try:
    ...
    PYDECK_DISPONIBLE = True
except ModuleNotFoundError:
    pdk = None
    PYDECK_DISPONIBLE = False
...
def ruta_carretera_osrm(lat1: float, lon1: float, lat2: float, lon2: float) -> Dict[str, Any]:
    """Calcula distancia, duración y geometría vial con el servidor público de OSRM."""
    if not REQUESTS_DISPONIBLE:
...
def _trayectoria_gcd(
    lat1: float, lon1: float, lat2: float, lon2: float, pasos: int = 80
) -> List[List[float]]:
    """Genera una polilínea aproximada de gran círculo para visualización cartográfica."""
    pasos = max(int(pasos), 2)
    phi1, lam1 = math.radians(lat1), math.radians(lon1)
    phi2, lam2 = math.radians(lat2), math.radians(lon2)
    x1 = (math.cos(phi1) * math.cos(lam1), math.cos(phi1) * math.sin(lam1), math.sin(phi1))
    x2 = (math.cos(phi2) * math.cos(lam2), math.cos(phi2) * math.sin(lam2), math.sin(phi2))
...
def calcular_distancia_automatica(
    ...
    if modo == "Carretero":
        ...
        trayectoria = ruta.get("trayectoria") or [[ori["lon"], ori["lat"]], [des["lon"], des["lat"]]]
    else:
        distancia = gcd
        duracion_h = None
        trayectoria = _trayectoria_gcd(ori["lat"], ori["lon"], des["lat"], des["lon"])
        if modo == "Aéreo":
...
def generar_pdf(
    ...
    with PdfPages(buffer) as pdf:
        ...
        # Página descriptiva: lectura automática y explicación del cálculo.
        fig = plt.figure(figsize=(11.69, 8.27))
        ax = fig.add_axes([0.04, 0.05, 0.92, 0.90])
        ax.axis("off")
        ax.text(0.0, 0.98, "Informe descriptivo de la operación", fontsize=16, fontweight="bold", va="top")

        y = 0.92
        lecturas = _lectura_automatica(df, por_modo, resumen)
        ax.text(0.0, y, "Lectura automática", fontsize=11, fontweight="bold", va="top")
        y -= 0.045
        for lectura in lecturas[:6]:
            texto = textwrap.fill(f"• {lectura}", width=125)
            ax.text(0.015, y, texto, fontsize=8.5, va="top", wrap=True)
            y -= 0.032 * max(1, texto.count("\n") + 1) + 0.015

        # Esquema geográfico de los puntos disponibles. No sustituye un mapa de red.
        filas_geo = df.dropna(subset=["origen_lat", "origen_lon", "destino_lat", "destino_lon"], how="any") \
            if all(c in df.columns for c in ["origen_lat", "origen_lon", "destino_lat", "destino_lon"]) else pd.DataFrame()
        if not filas_geo.empty:
            ax_map = fig.add_axes([0.60, 0.54, 0.34, 0.30])
            for _, r in filas_geo.iterrows():
                tray = r.get("trayectoria_mapa")
                if isinstance(tray, list) and len(tray) >= 2:
                    xs = [p[0] for p in tray if isinstance(p, (list, tuple)) and len(p) >= 2]
                    ys = [p[1] for p in tray if isinstance(p, (list, tuple)) and len(p) >= 2]
                    if len(xs) >= 2:
                        ax_map.plot(xs, ys, linewidth=1.4)
                else:
                    ax_map.plot([r["origen_lon"], r["destino_lon"]], [r["origen_lat"], r["destino_lat"]], linewidth=1.4)
                ax_map.scatter([r["origen_lon"], r["destino_lon"]], [r["origen_lat"], r["destino_lat"]], s=18)
                n = int(r.get("segmento", 0))
                ax_map.annotate(f"O{n}", (r["origen_lon"], r["origen_lat"]), fontsize=7)
                ax_map.annotate(f"D{n}", (r["destino_lon"], r["destino_lat"]), fontsize=7)
            ax_map.set_title("Esquema geográfico del recorrido", fontsize=9)
            ax_map.set_xlabel("Longitud", fontsize=7)
            ax_map.set_ylabel("Latitud", fontsize=7)
            ax_map.tick_params(labelsize=6)

        y = min(y, 0.49)
        ax.text(0.0, y, "Detalle y ecuación por tramo", fontsize=11, fontweight="bold", va="top")
        y -= 0.045
        for _, r in df.head(5).iterrows():
            cab = (
                f"Tramo {int(r.get('segmento', 0))}: {r.get('origen', '')} → {r.get('destino', '')} | "
                f"{r.get('modo', '')} | {float(r.get('distancia_carga_km', 0) or 0):,.1f} km | "
                f"{float(r.get('emision_total_kgco2e', 0) or 0):,.2f} kg CO₂e"
            )
            ax.text(0.0, y, textwrap.fill(cab, width=120), fontsize=8.5, fontweight="bold", va="top", wrap=True)
            y -= 0.035
            explicacion = textwrap.fill(_explicacion_calculo_tramo(r), width=125)
            ax.text(0.015, y, explicacion, fontsize=8, va="top", wrap=True)
            y -= 0.031 * max(1, explicacion.count("\n") + 1) + 0.022
            if y < 0.08:
                break
        ...
        ax.text(
            0.0,
            0.02,
            "El esquema geográfico muestra los puntos y trayectorias disponibles en los datos del tramo. Para ferrocarril, marítimo y fluvial, una línea GCD de referencia no representa necesariamente la red o ruta operacional real.",
            fontsize=7.5,
            va="bottom",
            wrap=True,
        )
...
def formulario_segmento(catalogo: pd.DataFrame) -> None:
    ...
    mensaje_distancia = st.session_state.get("mensaje_distancia_calculada", "")
    if mensaje_distancia:
        st.success(mensaje_distancia)
        st.session_state["mensaje_distancia_calculada"] = ""
    ...
    if ingreso_distancia == "Calcular automáticamente":
        ...
        if candidata and candidata.get("firma") == firma_actual:
            ...
            with st.expander("Ver recorrido en el mapa", expanded=True):
                _mostrar_mapa_tramos([
                    {
                        "segmento": 1,
                        "modo": modo,
                        "origen": origen,
                        "destino": destino,
                        "origen_lat": candidata.get("origen_lat"),
                        "origen_lon": candidata.get("origen_lon"),
                        "destino_lat": candidata.get("destino_lat"),
                        "destino_lon": candidata.get("destino_lon"),
                        "trayectoria_mapa": candidata.get("trayectoria"),
            ...
            st.caption(
                "Ingrese origen y destino y pulse **Calcular distancia** para revisar el valor y el mapa antes de agregar el tramo. "
                "Si intenta agregar el tramo sin haber pulsado el botón, la herramienta calculará la distancia automáticamente y "
                "volverá a mostrar el formulario con el resultado para su verificación."
            )
    else:
    ...
    if submitted:
        if ingreso_distancia == "Calcular automáticamente" and ruta_info is None:
            if not origen.strip() or not destino.strip():
                st.error("Ingrese origen y destino para calcular automáticamente la distancia.")
                return
            try:
                with st.spinner("Localizando los puntos y calculando la distancia automáticamente..."):
                    ruta_calculada = calcular_distancia_automatica(
                        origen, destino, modo, priorizar_uy
                    )
                st.session_state["ruta_automatica_nueva"] = ruta_calculada
                st.session_state["mensaje_distancia_calculada"] = (
                    f"Distancia calculada automáticamente: {ruta_calculada['distancia_km']:,.1f} km. "
                    "Revise el mapa y el método de distancia y vuelva a pulsar Agregar tramo a la operación."
                )
                # En transporte aéreo la distancia determina la categoría corto/largo alcance.
                # Se limpia cualquier selección previa para que el perfil recomendado se actualice al recargar.
                st.session_state.pop(f"nuevo_perfil_{modo}_{metodo_objetivo}", None)
                st.rerun()
            except Exception as exc:
                st.error(
                    f"No fue posible calcular la distancia automáticamente: {exc}. "
        ...
        ruta_campos = {
            ...
            "trayectoria_mapa": ruta_info.get("trayectoria") if ruta_info else None,
...
def gestionar_segmentos() -> None:
    ...
    with st.expander("Mapa del recorrido", expanded=True):
        _mostrar_mapa_tramos(segmentos, altura=430)
...
def _coords_validas(seg: Dict[str, Any]) -> bool:
    return all(
        seg.get(k) is not None
        for k in ("origen_lat", "origen_lon", "destino_lat", "destino_lon")
    )


def _completar_coords_para_mapa(seg: Dict[str, Any]) -> Dict[str, Any]:
    """Obtiene coordenadas solo para visualización cuando el tramo fue ingresado manualmente."""
    dato = dict(seg)
    if _coords_validas(dato):
        return dato
    if not REQUESTS_DISPONIBLE:
        return dato
    origen = _str(dato.get("origen")).strip()
    destino = _str(dato.get("destino")).strip()
    modo = _str(dato.get("modo")).strip()
    if not origen or not destino or not modo:
        return dato
    try:
        ori = geocodificar_lugar(origen, modo, True)
        des = geocodificar_lugar(destino, modo, True)
    except Exception:
        return dato
    ...
    dato.update({
        ...
        "destino_lat": des["lat"],
        "destino_lon": des["lon"],
        "origen_resuelto": dato.get("origen_resuelto") or ori.get("direccion", ""),
        "destino_resuelto": dato.get("destino_resuelto") or des.get("direccion", ""),
    })
    if not dato.get("trayectoria_mapa"):
        dato["trayectoria_mapa"] = _trayectoria_gcd(
            ori["lat"], ori["lon"], des["lat"], des["lon"]
        )
    return dato


def _zoom_mapa(puntos: List[Tuple[float, float]]) -> float:
    if not puntos:
        return 2.0
...
def _mostrar_mapa_tramos(segmentos: List[Dict[str, Any]], altura: int = 520) -> None:
    """Muestra los tramos almacenados. En carretera usa geometría OSRM cuando está disponible."""
    preparados: List[Dict[str, Any]] = []
    for i, seg in enumerate(segmentos, start=1):
        dato = _completar_coords_para_mapa(seg)
        dato.setdefault("segmento", i)
        preparados.append(dato)

    puntos: List[Dict[str, Any]] = []
    rutas: List[Dict[str, Any]] = []
    coords_centro: List[Tuple[float, float]] = []
    colores = {
        "Carretero": [220, 70, 70],
        "Ferroviario": [120, 80, 180],
        "Marítimo": [30, 110, 200],
        "Fluvial": [20, 150, 160],
        "Aéreo": [230, 140, 30],
    }

    for i, seg in enumerate(preparados, start=1):
        if not _coords_validas(seg):
            continue
        modo = _str(seg.get("modo"))
        ori_lat, ori_lon = float(seg["origen_lat"]), float(seg["origen_lon"])
        des_lat, des_lon = float(seg["destino_lat"]), float(seg["destino_lon"])
        coords_centro.extend([(ori_lat, ori_lon), (des_lat, des_lon)])
        puntos.extend([
            {
                "lon": ori_lon,
                "lat": ori_lat,
                "titulo": f"Tramo {i} · Origen",
                "subtitulo": _str(seg.get("origen")) or "Origen",
                "detalle": modo,
                "etiqueta": f"O{i}",
                "color": colores.get(modo, [80, 80, 80]),
            },
            {
                "lon": des_lon,
                "lat": des_lat,
                "titulo": f"Tramo {i} · Destino",
                "subtitulo": _str(seg.get("destino")) or "Destino",
                "detalle": modo,
                "etiqueta": f"D{i}",
                "color": colores.get(modo, [80, 80, 80]),
            },
        ])
        tray = seg.get("trayectoria_mapa") or []
        if not tray:
            tray = _trayectoria_gcd(ori_lat, ori_lon, des_lat, des_lon)
        rutas.append({
            "path": tray,
            "color": colores.get(modo, [80, 80, 80]),
            "titulo": f"Tramo {i} · {modo}",
            "subtitulo": f"{_str(seg.get('origen'))} → {_str(seg.get('destino'))}",
            "detalle": f"{_num(seg.get('distancia_carga_km')):,.1f} km",
        })

    if not rutas:
        st.info(
            "No fue posible geolocalizar los tramos para construir el mapa. "
            "Revise los nombres de origen y destino o utilice el cálculo automático de distancia."
        )
        return

    if PYDECK_DISPONIBLE:
        lat_c = sum(p[0] for p in coords_centro) / len(coords_centro)
        lon_c = sum(p[1] for p in coords_centro) / len(coords_centro)
        ...
        deck = pdk.Deck(
            ...
            initial_view_state=pdk.ViewState(
                latitude=lat_c,
                longitude=lon_c,
                zoom=_zoom_mapa(coords_centro),
                pitch=0,
            ),
            tooltip={
                "html": "<b>{titulo}</b><br/>{subtitulo}<br/>{detalle}",
                "style": {"backgroundColor": "#243447", "color": "white"},
            },
        )
        try:
            st.pydeck_chart(deck, use_container_width=True, height=altura)
        except TypeError:
            # Compatibilidad con versiones de Streamlit que no aceptan el argumento height.
            st.pydeck_chart(deck, use_container_width=True)
    else:
        st.map(pd.DataFrame({"lat": [p["lat"] for p in puntos], "lon": [p["lon"] for p in puntos]}))
        st.caption("Instale pydeck para visualizar también las líneas de cada recorrido.")

    modos_presentes = list(dict.fromkeys(_str(seg.get("modo")) for seg in preparados if _str(seg.get("modo"))))
    st.caption("Modos representados en el mapa: " + ", ".join(modos_presentes) + ".")
    if any(_str(seg.get("modo")) in {"Ferroviario", "Marítimo", "Fluvial"} for seg in preparados):
        st.caption(
            "En ferroviario, marítimo y fluvial la línea mostrada es una referencia geográfica cuando no se dispone de una geometría operacional. "
            "La distancia utilizada en el cálculo sigue siendo la registrada en cada tramo."
        )



def _lectura_automatica(
    df: pd.DataFrame,
    por_modo: pd.DataFrame,
    resumen: Dict[str, Any],
) -> List[str]:
    """Genera una lectura descriptiva basada únicamente en los resultados calculados."""
    if df.empty:
        return []

    lecturas: List[str] = []
    total_emisiones = _num(resumen.get("emisiones_totales_kgco2e"))
    distancia_total = _num(df["distancia_carga_km"].sum()) if "distancia_carga_km" in df.columns else 0.0

    if not por_modo.empty:
        dominante = por_modo.iloc[0]
        lecturas.append(
            f"El modo con mayor contribución es {_str(dominante.get('modo'))}, con "
            f"{_num(dominante.get('participacion_pct')):,.1f}% de las emisiones operativas de la operación."
        )

    if "intensidad_resultante_g_tkm" in df.columns and not df["intensidad_resultante_g_tkm"].empty:
        idx = df["intensidad_resultante_g_tkm"].astype(float).idxmax()
        tramo_intenso = df.loc[idx]
        lecturas.append(
            f"El tramo de mayor intensidad es el tramo {int(_num(tramo_intenso.get('segmento')))}, "
            f"{_str(tramo_intenso.get('origen'))} → {_str(tramo_intenso.get('destino'))}, con "
            f"{_num(tramo_intenso.get('intensidad_resultante_g_tkm')):,.2f} g CO₂e/tkm."
        )

    if "emision_total_kgco2e" in df.columns and total_emisiones > 0:
        idx = df["emision_total_kgco2e"].astype(float).idxmax()
        tramo_emisor = df.loc[idx]
        participacion = _num(tramo_emisor.get("emision_total_kgco2e")) / total_emisiones * 100.0
        if len(df) > 1:
            lecturas.append(
                f"El tramo que más contribuye al total es el tramo {int(_num(tramo_emisor.get('segmento')))}, "
                f"con {participacion:,.1f}% de las emisiones contabilizadas."
            )

    if "clase_dato" in df.columns:
        n_default = int((df["clase_dato"] == "Predeterminado").sum())
        if n_default:
            lecturas.append(
                f"{n_default} tramo(s) utilizan datos por defecto. Son adecuados como referencia cuando no existen datos específicos, "
                "pero constituyen la principal oportunidad para mejorar la precisión del estudio si posteriormente se dispone de datos primarios."
            )

    if "modo_ingreso_distancia" in df.columns:
        n_auto = int((df["modo_ingreso_distancia"] == "Calcular automáticamente").sum())
        n_manual = int(len(df) - n_auto)
        if n_auto and n_manual:
            lecturas.append(
                f"La operación combina {n_auto} tramo(s) con distancia calculada automáticamente y {n_manual} tramo(s) con distancia informada manualmente."
            )
        elif n_auto:
            lecturas.append(
                f"La distancia se obtuvo automáticamente en {n_auto} tramo(s). En carretera se usa la ruta vial OSRM; "
                "en aéreo se usa GCD y en ferroviario, marítimo y fluvial la distancia automática es una referencia geográfica."
            )

    if resumen.get("numero_modos", 0) > 1 and distancia_total > 0 and not por_modo.empty:
        comparacion = por_modo.copy()
        comparacion["participacion_distancia_pct"] = (
            comparacion["distancia_carga_km"].astype(float) / distancia_total * 100.0
        )
        comparacion["diferencia_emision_distancia"] = (
            comparacion["participacion_pct"].astype(float)
            - comparacion["participacion_distancia_pct"]
        )
        fila = comparacion.loc[comparacion["diferencia_emision_distancia"].idxmax()]
        if _num(fila.get("diferencia_emision_distancia")) > 5.0:
            lecturas.append(
                f"{_str(fila.get('modo'))} concentra una proporción de emisiones superior a su participación en la distancia total: "
                f"{_num(fila.get('participacion_pct')):,.1f}% de las emisiones frente a "
                f"{_num(fila.get('participacion_distancia_pct')):,.1f}% de la distancia."
            )
    ...
    if len(bases) > 1:
        lecturas.append(
            "Los factores empleados registran más de una base GWP en su trazabilidad. "
            "La herramienta conserva los valores publicados por cada fuente y señala esta heterogeneidad para que sea considerada al interpretar el agregado."
        )

    return lecturas


def _explicacion_calculo_tramo(r: pd.Series) -> str:
    """Explica con los valores del tramo la ecuación realmente aplicada."""
    metodo = _str(r.get("metodo_calculo"))
    emision = _num(r.get("emision_total_kgco2e"))
    masa = _num(r.get("masa_t"))
    distancia = _num(r.get("distancia_carga_km"))
    actividad_tkm = _num(r.get("actividad_tkm"))

    if metodo == "Intensidad de emisión":
        intensidad_teu = _num(r.get("intensidad_ttw_g_teukm"))
        teu = _num(r.get("teu_equivalente"))
        if intensidad_teu > 0 and teu > 0:
            actividad_teu = _num(r.get("actividad_teukm"))
            return (
                f"Se aplicó E = TEU × distancia × intensidad / 1000. "
                f"Con {teu:,.2f} TEU × {distancia:,.2f} km = {actividad_teu:,.2f} TEU-km y "
                f"{intensidad_teu:,.2f} g CO₂e/TEU-km, el resultado es {emision:,.2f} kg CO₂e. "
                f"Como referencia para comparación entre modos, la actividad de la expedición es {masa:,.3f} t × "
                f"{distancia:,.2f} km = {actividad_tkm:,.2f} tkm."
            )
        intensidad = _num(r.get("intensidad_ttw_g_tkm"))
        return (
            f"Se aplicó E = masa × distancia × intensidad / 1000. "
            f"Con {masa:,.3f} t × {distancia:,.2f} km = {actividad_tkm:,.2f} tkm y "
            f"{intensidad:,.2f} g CO₂e/tkm, el resultado es {emision:,.2f} kg CO₂e."
        )

    if metodo == "Consumo energético":
        cantidad = _num(r.get("cantidad_factor_asignada"))
        unidad_factor = _str(r.get("unidad_factor"), _str(r.get("unidad_energia")))
        ef = _num(r.get("ef_ttw_kgco2e_unidad"))
        return (
            f"Se aplicó E = consumo asignado × factor operacional. "
            f"La cantidad convertida a la unidad original del factor fue {cantidad:,.3f} {unidad_factor} y "
            f"el factor aplicado fue {ef:,.5f} kg CO₂e/{unidad_factor}; por tanto, la emisión es {emision:,.2f} kg CO₂e. "
            f"La intensidad resultante se obtiene dividiendo esa emisión por {actividad_tkm:,.2f} tkm."
        )

    return "No fue posible reconstruir la ecuación del tramo con la información registrada."


def _texto_informe_operacion(
    proyecto: Dict[str, str], df: pd.DataFrame, por_modo: pd.DataFrame, resumen: Dict[str, Any]
) -> str:
    if df.empty:
        return ""
    inicio = _str(df.iloc[0].get("origen"))
    fin = _str(df.iloc[-1].get("destino"))
    distancia_total = float(df["distancia_carga_km"].sum())
    tipo = "unimodal" if resumen["numero_modos"] == 1 else "multimodal"
    lineas = [
        f"# Informe de la operación de transporte",
        "",
        f"Proyecto: {proyecto.get('proyecto', '') or 'No informado'}",
        f"Empresa u organización: {proyecto.get('empresa', '') or 'No informada'}",
        f"Producto: {proyecto.get('producto', '') or 'No informado'}",
        f"Fecha: {datetime.now().strftime('%d/%m/%Y %H:%M')}",
        "",
        "## Resumen ejecutivo",
        f"La operación analizada es {tipo}, con {resumen['numero_segmentos']} tramo(s) y {resumen['numero_modos']} modo(s) de transporte.",
        f"El recorrido comienza en {inicio} y finaliza en {fin}, con {distancia_total:,.2f} km acumulados con carga.",
        f"Las emisiones operativas contabilizadas ascienden a {resumen['emisiones_totales_kgco2e']:,.2f} kg CO₂e.",
        f"La actividad acumulada es {resumen['actividad_total_tkm']:,.2f} tkm y la intensidad agregada es {resumen['intensidad_cadena_gco2e_tkm']:,.2f} g CO₂e/tkm.",
        "",
        "## Lectura automática",
    ]
    for lectura in _lectura_automatica(df, por_modo, resumen):
        lineas.extend([lectura, ""])
    lineas.extend([
        "## Detalle del recorrido",
    ])
    total = max(float(resumen["emisiones_totales_kgco2e"]), 0.0)
    for _, r in df.iterrows():
        part = (float(r.get("emision_total_kgco2e", 0.0)) / total * 100.0) if total > 0 else 0.0
        lineas.extend([
            "",
            f"### Tramo {int(r['segmento'])}: {_str(r.get('origen'))} → {_str(r.get('destino'))}",
            f"Modo: {_str(r.get('modo'))}",
            f"Distancia: {_num(r.get('distancia_carga_km')):,.2f} km",
            f"Masa de expedición: {_num(r.get('masa_t')):,.3f} t",
            f"Unidad de carga: {_str(r.get('unidad_carga'))}",
            f"Actividad: {_num(r.get('actividad_tkm')):,.2f} tkm",
            f"Método de cálculo: {_str(r.get('metodo_calculo'))}",
            f"Perfil aplicado: {_str(r.get('perfil_factor')) or 'Factor documentado por el usuario'}",
            f"Emisión del tramo: {_num(r.get('emision_total_kgco2e')):,.2f} kg CO₂e ({part:,.1f}% del total)",
            f"Intensidad resultante: {_num(r.get('intensidad_resultante_g_tkm')):,.2f} g CO₂e/tkm",
            f"Método de distancia: {_str(r.get('metodo_distancia')) or _str(r.get('modo_ingreso_distancia'))}",
            f"Explicación del cálculo: {_explicacion_calculo_tramo(r)}",
        ])
        if _str(r.get("observacion")):
            lineas.append(f"Observaciones: {_str(r.get('observacion'))}")
        if _str(r.get("nota_distancia")):
            lineas.append(f"Nota sobre la distancia: {_str(r.get('nota_distancia'))}")

    lineas.extend(["", "## Resumen por modo"])
    for _, r in por_modo.iterrows():
        lineas.append(
            f"{_str(r.get('modo'))}: {_num(r.get('distancia_carga_km')):,.2f} km, "
            f"{_num(r.get('emisiones_kgco2e')):,.2f} kg CO₂e, "
            f"{_num(r.get('participacion_pct')):,.1f}% del total."
        )
    dominante = por_modo.iloc[0] if not por_modo.empty else None
    lineas.extend([
        "",
        "## Interpretación y alcance",
        (
            f"El modo con mayor contribución es {_str(dominante.get('modo'))}, con "
            f"{_num(dominante.get('emisiones_kgco2e')):,.2f} kg CO₂e, equivalentes a "
            f"{_num(dominante.get('participacion_pct')):,.1f}% de las emisiones contabilizadas."
            if dominante is not None else ""
        ),
        "El resultado cuantifica exclusivamente las emisiones generadas durante la operación del transporte.",
        "Puertos, aeropuertos, terminales ferroviarias y demás nodos logísticos se registran como puntos de conexión, pero sus emisiones quedan fuera de la frontera cuantitativa.",
        "Los resultados deben interpretarse considerando la calidad de los datos de actividad, la representatividad del factor aplicado y el método utilizado para determinar la distancia de cada tramo.",
    ])
    return "\n".join(x for x in lineas if x is not None)


def _mostrar_informe_operacion(
    proyecto: Dict[str, str], segmentos: List[Dict[str, Any]], df: pd.DataFrame,
    por_modo: pd.DataFrame, resumen: Dict[str, Any]
) -> None:
    st.subheader("Informe de la operación")
    st.markdown("**Mapa del recorrido**")
    _mostrar_mapa_tramos(segmentos, altura=520)
    ...
    tipo = "unimodal" if resumen["numero_modos"] == 1 else "multimodal"
    st.markdown(
        f"La operación configurada es **{tipo}**, comprende **{resumen['numero_segmentos']} tramo(s)** y "
        f"**{resumen['numero_modos']} modo(s)**. El recorrido se extiende desde **{inicio}** hasta **{fin}**, "
        f"con **{distancia_total:,.2f} km** acumulados con carga. Las emisiones operativas totalizan "
        f"**{resumen['emisiones_totales_kgco2e']:,.2f} kg CO₂e**, para una actividad de "
        f"**{resumen['actividad_total_tkm']:,.2f} tkm** y una intensidad agregada de "
        f"**{resumen['intensidad_cadena_gco2e_tkm']:,.2f} g CO₂e/tkm**."
    )
    ...
    st.markdown("#### Detalle del trayecto")
    total = max(float(resumen["emisiones_totales_kgco2e"]), 0.0)
    for _, r in df.iterrows():
        n = int(r["segmento"])
        part = (_num(r.get("emision_total_kgco2e")) / total * 100.0) if total > 0 else 0.0
        with st.container(border=True):
            st.markdown(f"**Tramo {n}: {_str(r.get('origen'))} → {_str(r.get('destino'))}**")
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("Modo", _str(r.get("modo")))
            c2.metric("Distancia", f"{_num(r.get('distancia_carga_km')):,.1f} km")
            c3.metric("Emisión", f"{_num(r.get('emision_total_kgco2e')):,.2f} kg CO₂e")
            c4.metric("Participación", f"{part:,.1f}%")
            st.markdown(
                f"Masa de expedición: **{_num(r.get('masa_t')):,.3f} t**. "
                f"Unidad de carga: **{_str(r.get('unidad_carga'))}**. "
                f"Actividad: **{_num(r.get('actividad_tkm')):,.2f} tkm**. "
            ...
            with st.expander("Cómo se calculó este tramo", expanded=False):
                st.write(_explicacion_calculo_tramo(r))
            if _str(r.get("observacion")):
                st.caption("Observaciones: " + _str(r.get("observacion")))
            if _str(r.get("nota_distancia")):
                st.caption("Nota sobre la distancia: " + _str(r.get("nota_distancia")))

    st.markdown("#### Síntesis por modo")
    modo_view = por_modo.rename(columns={
        "modo": "Modo",
        "segmentos": "Tramos",
    ...
    cols = [c for c in ["Modo", "Tramos", "Distancia [km]", "Actividad [tkm]", "Emisiones [kg CO₂e]", "Intensidad [g CO₂e/tkm]", "Participación [%]"] if c in modo_view.columns]
    st.dataframe(modo_view[cols], use_container_width=True, hide_index=True)

    if not por_modo.empty:
        dom = por_modo.iloc[0]
        st.markdown(
            f"El modo con mayor contribución es **{_str(dom.get('modo'))}**, con "
            f"**{_num(dom.get('emisiones_kgco2e')):,.2f} kg CO₂e**, equivalentes a "
            f"**{_num(dom.get('participacion_pct')):,.1f}%** del total contabilizado."
        )

    st.info(
        "El informe cuantifica exclusivamente emisiones durante la operación del transporte. "
        "Los nodos logísticos se consideran puntos de transferencia y quedan fuera de la cuantificación."
    )
    texto = _texto_informe_operacion(proyecto, df, por_modo, resumen)
    st.download_button(
        "Descargar informe descriptivo",
        data=texto.encode("utf-8"),
        file_name="informe_operacion_transporte.md",
        mime="text/markdown",
    )
...
def panel_resultados(proyecto: Dict[str, str]) -> None:
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(
        ["Por tramo", "Por modo", "Calidad de datos y nivel IPCC", "Trazabilidad", "Informe de la operación"]
    )
    ...
    with tab5:
        _mostrar_informe_operacion(proyecto, segmentos, df, por_modo, resumen)
