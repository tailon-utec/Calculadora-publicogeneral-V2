...
def formulario_segmento(catalogo: pd.DataFrame) -> None:
    ...
    for _, fila in filas_modo.iterrows():
        ...
        if modo == "Marítimo" and metodo_objetivo == "Intensidad de emisión":
            # Evita mezclar perfiles de contenedores con perfiles de buques no contenerizados.
    ...
    with st.expander("¿Qué diferencia hay entre los perfiles?", expanded=False):
        st.markdown(
            ...
            **Factor propio documentado**  
            Es una alternativa avanzada. Utilícelo únicamente cuando su empresa disponga de un factor propio, contractual, oficial o técnicamente documentado. Si existe un perfil integrado compatible, no es necesario ingresar ningún factor manual.
            """
        )
    ...
    if metodo_objetivo == "Intensidad de emisión":
        if modo == "Marítimo" and unidad_carga == "Contenedor ISO":
            ...
            perfil_recomendado = "GLEC v3.2 | Carretero | Camión pesado >20 t | Sudamérica/Europa | Intensidad estándar"
        elif modo == "Ferroviario":
            perfil_recomendado = "GLEC v3.2 | Ferroviario | Tracción no identificada | Referencia internacional"
        elif modo == "Aéreo":
            perfil_recomendado = (
        ...
        # Para cadenas multimodales se prioriza WTW cuando existe un perfil GLEC compatible,
        # ya que permite agregar los distintos modos bajo una frontera homogénea.
        recomendados_consumo = {
            "Carretero": "GLEC v3.2 | Carretero | Diésel | Emisiones de ciclo energético completo",
            "Ferroviario": "GLEC v3.2 | Ferroviario | Diésel | Emisiones de ciclo energético completo",
            "Marítimo": "GLEC v3.2 | Marítimo | MDO/MGO VLSFO | Emisiones de ciclo energético completo",
            "Fluvial": "GLEC v3.2 | Fluvial | MDO/MGO VLSFO | Emisiones de ciclo energético completo",
            "Aéreo": "GLEC v3.2 | Aéreo | Jet A1/A | Emisiones de ciclo energético completo",
        }
    ...
    if perfil_recomendado and perfil_recomendado in perfiles_modo:
        ...
        st.success(
            ...
            "No necesita ingresar WTT, TTW, WTW ni un factor de emisión manual."
        )
        ...
        st.warning(
            "No hay un factor integrado compatible con esta configuración. "
            "Para continuar deberá utilizar un factor documentado propio o modificar la configuración del tramo."
        )
