...
1. Perfiles operacionales TTW basados en fuentes oficiales utilizadas por Uruguay.
2. Perfiles WTW del Marco GLEC v3.2 como referencia internacional cuando corresponda.
3. Factores documentados cargados por el usuario o ingresados manualmente.
...
presentan como factores nacionales uruguayos. La herramienta impide agregar segmentos
con fronteras energéticas o bases de GWP incompatibles.
"""
...
METODOLOGIA = "ISO 14083:2023 | Marco GLEC v3.2 | Directrices IPCC 2006"
VERSION = "0.7.0"
...
#
# Los perfiles "GLEC v3.2" son referencias internacionales WTW por kg de
# combustible. No deben interpretarse como factores nacionales del Uruguay.
...
FACTORES_INTEGRADOS = pd.DataFrame(
    [
        ...
        {
            "perfil": "GLEC v3.2 | Marítimo contenedor | Promedio industria | Seco | WTW",
            "modo": "Marítimo",
        ...
        {
            "perfil": "GLEC v3.2 | Marítimo contenedor | Promedio industria | Reefer | WTW",
            "modo": "Marítimo",
            "metodo": "Intensidad de emisión",
            "intensidad_wtt_g_tkm": None,
            "intensidad_ttw_g_tkm": None,
        ...
        {
            "perfil": "GLEC v3.2 | Marítimo contenedor | Asia ↔ Sudamérica | Seco | WTW",
            "modo": "Marítimo",
            "metodo": "Intensidad de emisión",
            "intensidad_wtt_g_tkm": None,
            "intensidad_ttw_g_tkm": None,
        ...
        {
            "perfil": "GLEC v3.2 | Marítimo contenedor | Asia ↔ Sudamérica | Reefer | WTW",
            "modo": "Marítimo",
            "metodo": "Intensidad de emisión",
            "intensidad_wtt_g_tkm": None,
            "intensidad_ttw_g_tkm": None,
...
_RENOMBRE_PERFILES = {
    "Uruguay | Carretero | Gasoil | TTW": "Uruguay | Carretero | Gasoil | Emisiones operativas TTW",
    "Uruguay | Ferroviario | Gasoil | TTW": "Uruguay | Ferroviario | Gasoil | Emisiones operativas TTW",
    "Uruguay | Marítimo | Gasoil marino | TTW": "Uruguay | Marítimo | Gasoil marino | Emisiones operativas TTW",
    "Uruguay | Marítimo | Fueloil pesado | TTW": "Uruguay | Marítimo | Fueloil pesado | Emisiones operativas TTW",
    "Uruguay | Fluvial | Gasoil marino | TTW": "Uruguay | Fluvial | Gasoil marino | Emisiones operativas TTW",
    "Uruguay | Fluvial | Fueloil pesado | TTW": "Uruguay | Fluvial | Fueloil pesado | Emisiones operativas TTW",
    "Uruguay | Aéreo | Jet A1 | TTW": "Uruguay | Aéreo | Jet A1 | Emisiones operativas TTW",
    "GLEC v3.2 | Carretero | Diésel 100% | WTW": "GLEC v3.2 | Carretero | Diésel | Emisiones totales WTW",
    "GLEC v3.2 | Ferroviario | Diésel 100% | WTW": "GLEC v3.2 | Ferroviario | Diésel | Emisiones totales WTW",
    "GLEC v3.2 | Marítimo | MDO/MGO VLSFO | WTW": "GLEC v3.2 | Marítimo | MDO/MGO VLSFO | Emisiones totales WTW",
    "GLEC v3.2 | Marítimo | HFO VLSFO | WTW": "GLEC v3.2 | Marítimo | HFO VLSFO | Emisiones totales WTW",
    "GLEC v3.2 | Fluvial | MDO/MGO VLSFO | WTW": "GLEC v3.2 | Fluvial | MDO/MGO VLSFO | Emisiones totales WTW",
    "GLEC v3.2 | Aéreo | Jet A1/A | WTW": "GLEC v3.2 | Aéreo | Jet A1/A | Emisiones totales WTW",
}
...
_DENSIDADES_PERFIL = {
    "GLEC v3.2 | Carretero | Diésel | Emisiones totales WTW": (
        0.832,
        "Smart Freight Centre, GLEC Framework v3.2, factor europeo de diésel: densidad 0,832 kg/L.",
    ),
    "GLEC v3.2 | Ferroviario | Diésel | Emisiones totales WTW": (
        0.832,
        "Smart Freight Centre, GLEC Framework v3.2, factor europeo de diésel: densidad 0,832 kg/L.",
    ),
    "GLEC v3.2 | Aéreo | Jet A1/A | Emisiones totales WTW": (
        0.802,
        "Smart Freight Centre, GLEC Framework v3.2, Jet Kerosene: densidad 0,802 kg/L.",
    ),
    "GLEC v3.2 | Marítimo | MDO/MGO VLSFO | Emisiones totales WTW": (
        0.890,
        "ANCAP, ficha de seguridad Gasoil Marino: densidad 890 kg/m³ a 15 °C. Se usa solo para convertir L a kg.",
    ),
    "GLEC v3.2 | Fluvial | MDO/MGO VLSFO | Emisiones totales WTW": (
        0.890,
        "ANCAP, ficha de seguridad Gasoil Marino: densidad 890 kg/m³ a 15 °C. Se usa solo para convertir L a kg.",
    ),
    "GLEC v3.2 | Marítimo | HFO VLSFO | Emisiones totales WTW": (
        0.991,
...
def validar_catalogo_factores(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    mensajes: List[str] = []
    faltantes = [c for c in COLUMNAS_FACTORES if c not in df.columns]
    if faltantes:
        mensajes.append(
            "La base de factores no contiene todas las columnas requeridas: "
            + ", ".join(faltantes)
        )
        return pd.DataFrame(columns=COLUMNAS_FACTORES), mensajes

    limpio = df.copy()
    for columna in COLUMNAS_FACTORES_OPCIONALES:
        if columna not in limpio.columns:
            limpio[columna] = None
    limpio = limpio[COLUMNAS_FACTORES + COLUMNAS_FACTORES_OPCIONALES].copy()
    ...
    return limpio.reset_index(drop=True), mensajes
...
def calcular_segmento(seg: Dict[str, Any]) -> Dict[str, Any]:
    """Calcula las emisiones de un tramo de transporte y conserva su trazabilidad."""
    ...
    if metodo == "Intensidad de emisión":
        wtt = _num(seg.get("intensidad_wtt_g_tkm"))
        ttw = _num(seg.get("intensidad_ttw_g_tkm"))
        wtw = _num(seg.get("intensidad_wtw_g_tkm"))

        wtt_teu = _num(seg.get("intensidad_wtt_g_teukm"))
        ttw_teu = _num(seg.get("intensidad_ttw_g_teukm"))
        wtw_teu = _num(seg.get("intensidad_wtw_g_teukm"))

        usa_tkm = any(v > 0 for v in (wtt, ttw, wtw))
        usa_teu = any(v > 0 for v in (wtt_teu, ttw_teu, wtw_teu))
        if usa_tkm and usa_teu:
        ...
        if usa_teu:
            if teu_equivalente <= 0:
                raise ValueError(
                    ...
                    "para determinar los TEU equivalentes."
                )
            if wtw_teu > 0 and (wtt_teu > 0 or ttw_teu > 0):
                raise ValueError("No combine una intensidad WTW total con componentes WTT o TTW.")
            frontera = _determinar_frontera(wtt_teu, ttw_teu, wtw_teu)
            if not frontera:
                raise ValueError("Informe una intensidad por TEU-km válida.")
            if wtw_teu > 0:
                em_total = actividad_teukm * wtw_teu / 1000.0
                em_wtt = 0.0
                em_ttw = 0.0
            else:
                em_wtt = actividad_teukm * wtt_teu / 1000.0
                em_ttw = actividad_teukm * ttw_teu / 1000.0
                em_total = em_wtt + em_ttw
            intensidad_tkm_resultante = em_total / actividad_tkm * 1000.0 if actividad_tkm > 0 else 0.0
            resultado["unidad_intensidad_aplicada"] = "g CO₂e/TEU-km"
            resultado["intensidad_resultante_g_tkm"] = intensidad_tkm_resultante
        else:
            if wtw > 0 and (wtt > 0 or ttw > 0):
                raise ValueError(
                    "No combine una intensidad WTW total con componentes WTT o TTW. "
                    "Utilice una sola estructura de factor."
                )
            frontera = _determinar_frontera(wtt, ttw, wtw)
            if not frontera:
                raise ValueError("Informe una intensidad WTW o al menos un componente WTT o TTW.")
            if wtw > 0:
                em_total = actividad_tkm * wtw / 1000.0
                em_wtt = 0.0
                em_ttw = 0.0
                intensidad_tkm_resultante = wtw
            else:
                em_wtt = actividad_tkm * wtt / 1000.0
                em_ttw = actividad_tkm * ttw / 1000.0
                em_total = em_wtt + em_ttw
                intensidad_tkm_resultante = wtt + ttw
            resultado["unidad_intensidad_aplicada"] = "g CO₂e/tkm"
            resultado["intensidad_resultante_g_tkm"] = intensidad_tkm_resultante

        resultado["emision_wtt_kgco2e"] = em_wtt
        resultado["emision_ttw_kgco2e"] = em_ttw
        resultado["emision_total_kgco2e"] = em_total
        resultado["frontera_calculo"] = frontera
        ...
        if forma_consumo in {
            "Cantidad total",
            "Cantidad total del tramo",
            "Consumo total del tramo",
            "Litros totales consumidos",
        }:
            q_bruto = _num(seg.get("cantidad_energia_total"))
        elif forma_consumo in {"Consumo por 100 km", "Consumo medio en L/100 km"}:
            cons_carga_100 = _num(seg.get("consumo_por_100km_carga"))
            cons_vacio_100 = _num(seg.get("consumo_por_100km_vacio"))
            q_bruto = (
                cons_carga_100 / 100.0 * distancia_carga_km
                + cons_vacio_100 / 100.0 * distancia_vacia_km
            )
            ...
            if distancia_vacia_km > 0:
                if rend_vacio <= 0:
                    raise ValueError(
                        "Informe el rendimiento en vacío cuando exista una distancia vacía asociada."
                    )
                q_bruto += distancia_vacia_km / rend_vacio
        else:
            cons_carga = _num(seg.get("consumo_por_km_carga"))
            cons_vacio = _num(seg.get("consumo_por_km_vacio"))
            q_bruto = cons_carga * distancia_carga_km + cons_vacio * distancia_vacia_km
        ...
        ef_wtt = _num(seg.get("ef_wtt_kgco2e_unidad"))
        ...
        if ef_wtt <= 0 and ef_ttw <= 0:
            raise ValueError("Informe al menos un factor de emisión WTT o TTW.")
        if not unidad_usuario:
            raise ValueError("Informe la unidad de consumo utilizada.")

        frontera = _determinar_frontera(ef_wtt, ef_ttw)
        em_wtt = q_factor_asignado * ef_wtt
        ...
        resultado["emision_ttw_kgco2e"] = em_ttw
        resultado["emision_total_kgco2e"] = em_total
        resultado["intensidad_resultante_g_tkm"] = (
            em_total / actividad_tkm * 1000.0 if actividad_tkm > 0 else 0.0
        )
        resultado["frontera_calculo"] = frontera
...
def calcular_cadena(segmentos: List[Dict[str, Any]]) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, Any]]:
    ...
    if len(bases_gwp) > 1:
        raise ValueError(
            "La cadena combina bases de potencial de calentamiento global incompatibles: "
            + ", ".join(sorted(bases_gwp))
            + ". Utilice una misma base GWP para todos los segmentos."
        )
    ...
    resumen = {
        ...
        "base_gwp": next(iter(bases_gwp), "No informada"),
    }
...
def generar_pdf(
    ...
    with PdfPages(buffer) as pdf:
        ...
        kpi = (
            ...
            f"Frontera energética: {resumen['frontera_energetica']}   Base GWP: {resumen['base_gwp']}\n"
...
def cabecera() -> None:
    ...
    st.info(
        ...
        "2) indique la carga y el consumo de combustible o energía; "
        "3) agregue el tramo y repita hasta completar la cadena. "
...
def sidebar_factores() -> pd.DataFrame:
    ...
    with st.sidebar.expander("Base de factores personalizada", expanded=False):
        ...
        template = pd.DataFrame(columns=COLUMNAS_FACTORES + COLUMNAS_FACTORES_OPCIONALES)
        st.download_button(
    ...
    with st.sidebar.expander("Información metodológica", expanded=False):
        st.markdown(
            ...
            **Frontera energética**  
            Los perfiles nacionales integrados son TTW. Los perfiles GLEC integrados son WTW. La herramienta evita mezclar fronteras incompatibles en una misma cadena.

            **Base GWP**  
            Los perfiles nacionales integrados mantienen la base utilizada por la fuente nacional. Los perfiles GLEC se identifican con su propia base metodológica. La herramienta evita agregarlas cuando no son compatibles.
            """
        )
...
def formulario_segmento(catalogo: pd.DataFrame) -> None:
    ...
    if st.session_state["segmentos"]:
        try:
            primero = calcular_segmento(st.session_state["segmentos"][0])
            frontera_objetivo = _str(primero.get("frontera_calculo"))
            base_gwp_objetivo = _str(st.session_state["segmentos"][0].get("base_gwp"))
        except Exception:
            pass
    ...
    indice_info = 1 if modo == "Carretero" else 0
    disponibilidad_info = st.radio(
        ...
        help=(
            "Para transporte marítimo, aéreo, ferroviario o fluvial es habitual que el usuario del servicio no conozca "
            "el consumo total del vehículo. En esos casos se utiliza una intensidad de emisión documentada."
        ),
    )
    metodo_objetivo = (
        "Intensidad de emisión"
        if disponibilidad_info.startswith("No conozco")
        else "Consumo energético"
    )
    if modo == "Marítimo" and metodo_objetivo == "Intensidad de emisión":
        st.info(
            "En transporte marítimo normalmente el cargador no conoce el combustible consumido por el buque. "
            "El Marco GLEC permite utilizar datos modelados o por defecto de una categoría de operación. "
            "Para contenedores, los perfiles integrados se expresan por TEU-km."
        )

    filas_modo = catalogo[
        (catalogo["modo"] == modo) & (catalogo["metodo"] == metodo_objetivo)
    ].copy()
    perfiles_modo: List[str] = []
    for _, fila in filas_modo.iterrows():
        data = fila.to_dict()
        frontera = _frontera_perfil(data)
        base = _str(data.get("base_gwp"))
        if frontera_objetivo and frontera and frontera != frontera_objetivo:
            continue
        if base_gwp_objetivo and base and base != base_gwp_objetivo:
            continue
        usa_teu = any(
            _num(data.get(c)) > 0
            for c in ("intensidad_wtt_g_teukm", "intensidad_ttw_g_teukm", "intensidad_wtw_g_teukm")
        )
        if usa_teu and unidad_carga != "Contenedor ISO":
            continue
        perfiles_modo.append(_str(data.get("perfil")))

    if frontera_objetivo or base_gwp_objetivo:
        st.caption(
            f"La cadena ya está configurada con frontera **{frontera_objetivo or 'registrada'}** y base "
            f"**{base_gwp_objetivo or 'registrada'}**. Solo se muestran perfiles compatibles."
        )
    ...
    with st.expander("¿Qué diferencia hay entre los perfiles?", expanded=False):
        st.markdown(
            """
            **Perfil Uruguay · TTW**  
            Usa parámetros nacionales y representa las emisiones operativas asociadas al uso del combustible. No incluye la producción y suministro del combustible.

            **Perfil GLEC · WTW**  
            Es una referencia internacional. Incluye WTT, producción y suministro de la energía, más TTW, uso de la energía durante el transporte. Es más completo para comparar cadenas cuando todos los tramos utilizan la misma frontera.

            **Factor documentado manualmente**  
            Utilícelo cuando su empresa disponga de un factor propio, contractual, oficial o técnicamente documentado. La fuente y la frontera deben quedar registradas.
            """
        )

    perfil_recomendado_mar = ""
    if modo == "Marítimo" and metodo_objetivo == "Intensidad de emisión" and unidad_carga == "Contenedor ISO":
        es_reefer = condicion_contenedor.startswith("Refrigerado")
        perfil_recomendado_mar = (
            "GLEC v3.2 | Marítimo contenedor | Promedio industria | Reefer | WTW"
            if es_reefer
            else "GLEC v3.2 | Marítimo contenedor | Promedio industria | Seco | WTW"
        )

    if not perfiles_modo:
        if modo == "Marítimo" and metodo_objetivo == "Intensidad de emisión":
            st.warning(
                "No hay un perfil marítimo automático compatible con la configuración actual. "
                "Para carga contenerizada seleccione **Contenedor ISO**. Para carga a granel, Ro-Ro o carga general "
                "se requiere una intensidad documentada compatible con el tipo de servicio o buque."
            )
        else:
            st.caption(
                "No hay un perfil integrado para esta combinación de modo, método y frontera. "
                "Puede ingresar una intensidad o un factor documentado manualmente."
            )

    if perfil_recomendado_mar and perfil_recomendado_mar in perfiles_modo:
        otros_perfiles = [p for p in perfiles_modo if p != perfil_recomendado_mar]
        opciones_perfil = [perfil_recomendado_mar] + otros_perfiles + ["Entrada manual"]
        perfil = st.selectbox(
            "Perfil marítimo aplicado",
            opciones_perfil,
            index=0,
            key=f"nuevo_perfil_{modo}",
            format_func=lambda x: "Usar un factor propio documentado" if x == "Entrada manual" else x,
            help=(
                "La herramienta propone automáticamente un perfil GLEC compatible con transporte marítimo contenerizado. "
                "Puede cambiarlo si dispone de un perfil más representativo."
            ),
        )
        st.success(
            "No necesita informar el consumo del buque ni completar manualmente WTT, TTW o WTW. "
            "El cálculo utiliza la distancia, los TEU equivalentes y la intensidad documentada del perfil seleccionado."
        )
    else:
        opciones_perfil = ["Entrada manual"] + perfiles_modo
        perfil = st.selectbox(
            "Fuente o perfil del factor",
            opciones_perfil,
            key=f"nuevo_perfil_{modo}",
            format_func=lambda x: "Ingresar un factor documentado manualmente" if x == "Entrada manual" else x,
            help=(
                "Priorice un factor representativo de su operación. No compare ni agregue directamente segmentos TTW con WTW."
            ),
        )
    ...
    if perfil_data:
        ...
        if perfil.startswith("Uruguay"):
            st.success(
                f"**Perfil nacional · {frontera_perfil}.** Calcula emisiones operativas a partir del consumo del tramo."
            )
            st.caption(
                "TTW considera el uso del combustible durante la operación. No incluye la etapa de producción y suministro."
            )
        else:
            st.info(
                f"**Referencia internacional GLEC · {frontera_perfil}.** Incluye los componentes definidos por el perfil seleccionado."
            )
            if frontera_perfil == "WTW":
                st.caption("WTW = WTT, suministro de la energía, + TTW, uso de la energía durante el transporte.")
        with st.expander("Ver fuente y detalles del factor"):
            ...
            st.write(f"**Observación:** {perfil_data.get('observacion', '')}")
    ...
    if metodo_default == "Intensidad de emisión":
        ...
        st.caption(
            "Use esta opción cuando disponga de datos reales o confiables de consumo. Para combustibles líquidos la interfaz "
            "prioriza litros; si el factor original está expresado por kg, la conversión se realiza internamente mediante una densidad documentada."
        )
    ...
    if metodo == "Intensidad de emisión":
        ...
        if perfil_data:
            ...
            if any(v > 0 for v in (int_wtt_teu, int_ttw_teu, int_wtw_teu)):
                st.caption(
                    f"Intensidad aplicada: WTT {int_wtt_teu:.1f} | TTW {int_ttw_teu:.1f} | "
                    f"WTW {int_wtw_teu:.1f} g CO₂e/TEU-km."
                )
                if teu_equivalente <= 0:
                    st.warning(
                ...
                st.caption(
                    f"Intensidad aplicada: WTT {int_wtt:.2f} | TTW {int_ttw:.2f} | WTW {int_wtw:.2f} g CO₂e/tkm."
                )
        else:
            if modo == "Marítimo":
                st.info(
            ...
            if habilitar_factor_manual:
                ...
                if unidad_int_manual == "g CO₂e/TEU-km":
                    with d1:
                        int_wtt_teu = st.number_input("WTT [g CO₂e/TEU-km]", min_value=0.0, value=0.0, step=0.1)
                    with d2:
                        int_ttw_teu = st.number_input("TTW [g CO₂e/TEU-km]", min_value=0.0, value=0.0, step=0.1)
                    with d3:
                        int_wtw_teu = st.number_input(
                            "WTW total [g CO₂e/TEU-km]", min_value=0.0, value=0.0, step=0.1,
                            help="No combine WTW total con WTT o TTW separados.",
                        )
                else:
                    with d1:
                        int_wtt = st.number_input("WTT [g CO₂e/tkm]", min_value=0.0, value=0.0, step=0.1)
                    with d2:
                        int_ttw = st.number_input("TTW [g CO₂e/tkm]", min_value=0.0, value=0.0, step=0.1)
                    with d3:
                        int_wtw = st.number_input(
                            "WTW total [g CO₂e/tkm]", min_value=0.0, value=0.0, step=0.1,
                            help="No combine WTW total con WTT o TTW separados.",
                        )
            else:
        ...
        if perfil_data:
            ...
            if unidad_factor == "kg" and densidad > 0:
                st.info(
                    ...
                    f"La herramienta convierte L → kg con una densidad de **{densidad:.3f} kg/L** y mantiene el factor original sin modificar."
                )
                st.caption(f"Factor original: WTT {ef_wtt:.5f} y TTW {ef_ttw:.5f} kg CO₂e/kg.")
            else:
                st.caption(
                    f"Unidad de entrada: **{unidad}**. Factor aplicado: WTT {ef_wtt:.5f} y TTW {ef_ttw:.5f} kg CO₂e/{unidad_factor}."
                )
        else:
            ...
            c_wtt, c_ttw = st.columns(2)
            with c_wtt:
                ef_wtt = st.number_input(
                    f"Factor WTT [kg CO₂e/{unidad_factor}]", min_value=0.0, value=0.0, step=0.001, format="%.5f"
                )
            with c_ttw:
                ef_ttw = st.number_input(
                    f"Factor TTW [kg CO₂e/{unidad_factor}]", min_value=0.0, value=0.0, step=0.001, format="%.5f"
                )
    ...
    with st.expander("Opciones avanzadas y trazabilidad", expanded=perfil_data is None):
        ...
        clase_default = perfil_data.get("clase_dato") if perfil_data else "Primario"
        clase_index = CLASES_DATO.index(clase_default) if clase_default in CLASES_DATO else 0
        clase_dato = st.selectbox(
            "Clase de dato",
            CLASES_DATO,
            index=clase_index,
            format_func=lambda x: ETIQUETAS_CLASE_DATO.get(x, x),
            disabled=perfil_data is not None,
            help="Clasificación de la información utilizada según la lógica ISO 14083 / GLEC.",
        )

        tier_default = perfil_data.get("tier") if perfil_data else "No aplica"
        tier_index = TIERS.index(tier_default) if tier_default in TIERS else 0
        tier = st.selectbox(
            "Nivel metodológico IPCC",
            TIERS,
            index=tier_index,
            format_func=lambda x: ETIQUETAS_NIVEL_IPCC.get(x, x),
            disabled=perfil_data is not None,
        )

        fuente_default = _str(perfil_data.get("fuente")) if perfil_data else ""
        base_gwp_default = _str(perfil_data.get("base_gwp")) if perfil_data else ""
        anio_default = int(_num(perfil_data.get("anio"), datetime.now().year)) if perfil_data else datetime.now().year

        fuente = st.text_area(
            "Fuente del factor o dato",
            value=fuente_default,
            help="Registre documento oficial, DOI, base de datos u operador que sustenta el factor.",
            disabled=perfil_data is not None,
        )
        c_gwp, c_anio = st.columns(2)
        with c_gwp:
            base_gwp = st.text_input(
                "Base de GWP",
                value=base_gwp_default,
                placeholder="Ej.: IPCC AR6 GWP100",
                disabled=perfil_data is not None,
            )
        with c_anio:
    ...
    submitted = st.button("Agregar tramo a la operación", type="primary", use_container_width=True)
...
def gestionar_segmentos() -> None:
        ...
        [
            {
                ...
                "Dato": ETIQUETAS_CLASE_DATO.get(s.get("clase_dato", ""), s.get("clase_dato", "")),
                "Nivel IPCC": ETIQUETAS_NIVEL_IPCC.get(s.get("tier", ""), s.get("tier", "")),
            }
    ...
    with c1:
        if st.button("Eliminar último segmento"):
            ...
            st.rerun()
    with c2:
        if st.button("Eliminar toda la cadena"):
...
def panel_resultados(proyecto: Dict[str, str]) -> None:
    ...
    try:
        ...
        st.error(f"No fue posible calcular la cadena: {exc}")
        return
    ...
    st.caption(
        f"Frontera energética del resultado: {resumen['frontera_energetica']}. "
    ...
    with tab1:
        ...
        rename = {
            ...
            "tier": "Nivel IPCC",
            "base_gwp": "Base GWP",
            "frontera_calculo": "Frontera",
            "emision_wtt_kgco2e": "WTT [kg CO₂e]",
            "emision_ttw_kgco2e": "TTW [kg CO₂e]",
    ...
    with tab4:
        ...
        traz = traz.rename(columns={
            ...
            "tier": "Nivel IPCC",
            "base_gwp": "Base GWP",
            "frontera_calculo": "Frontera de cálculo",
    ...
    with st.expander("Notas metodológicas de la herramienta"):
        st.markdown(
            ...
            **2. Cálculo por intensidad**  
            La emisión se obtiene multiplicando la actividad del segmento por una intensidad expresada en g CO₂e/tkm. Puede utilizarse una intensidad WTW total o el desglose WTT correspondiente a la producción y suministro de energía y TTW correspondiente al uso de la energía durante la operación.

            **3. Cálculo por consumo energético**  
            La emisión se obtiene multiplicando la cantidad de energía asignada a la carga por los factores WTT y TTW compatibles con la unidad energética informada.
            ...
            **7. Coherencia de agregación**  
            La herramienta no agrega segmentos con fronteras energéticas diferentes ni con bases GWP incompatibles. Una cadena TTW debe permanecer TTW y una cadena WTW debe permanecer WTW.

            **8. Trazabilidad**  
            Cada segmento conserva la fuente, el año, la clase de dato, el nivel IPCC cuando corresponda, la base GWP, la frontera energética y los parámetros de cálculo.
            """
        )
