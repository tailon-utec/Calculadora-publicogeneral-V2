...
def validar_catalogo_factores(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    ...
    limpio = df[COLUMNAS_FACTORES].copy()
...
def calcular_segmento(seg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if metodo == "Intensidad de emisión":
        ...
        resultado["intensidad_resultante_g_tkm"] = intensidad
...
def sidebar_factores() -> pd.DataFrame:
    ...
    with st.sidebar.expander("Base de factores personalizada", expanded=False):
        ...
        template = pd.DataFrame(columns=COLUMNAS_FACTORES)
        st.download_button(
...
def formulario_segmento(catalogo: pd.DataFrame) -> None:
    ...
    filas_modo = catalogo[catalogo["modo"] == modo].copy()
    ...
    metodo_default = perfil_data.get("metodo") if perfil_data else "Consumo energético"
    metodo_index = METODOS_CALCULO.index(metodo_default) if metodo_default in METODOS_CALCULO else 1

    # ------------------------------------------------------------------
    # 3. Consumo de combustible o energía
    # ------------------------------------------------------------------
    st.markdown("### 3. Consumo de combustible o energía")
    st.caption(
        "Cuando el factor se basa en combustible, informe el consumo del tramo. Para combustibles líquidos la interfaz "
    ...
    if perfil_data is None:
        metodo = st.selectbox(
            ...
            index=metodo_index,
            help=(
                "Consumo energético: usa combustible o electricidad consumidos. Intensidad: usa un factor en g CO₂e/tkm."
            ),
        )
    else:
