...
Efecto Invernadero en Cadenas Multimodales de Transporte de Cargas en Uruguay
...
APP_TITLE = (
    ...
    "en Cadenas Multimodales de Transporte de Cargas en Uruguay"
)
APP_SHORT = "Emisiones GEI del transporte multimodal"
...
def formulario_segmento(catalogo: pd.DataFrame) -> None:
    ...
    st.caption(
        ...
        "La herramienta calcula cada tramo por separado y luego consolida la cadena multimodal."
    )
    ...
    if unidad_carga == "Contenedor ISO":
        st.markdown("**Equivalencia del contenedor para transporte marítimo**")
        ...
        st.caption(f"Equivalencia utilizada: **{teu_equivalente:.2f} TEU**.")
    ...
    submitted = st.button("Agregar tramo a la cadena", type="primary", use_container_width=True)
...
def gestionar_segmentos() -> None:
    ...
    if not segmentos:
        st.warning("Todavía no se incorporaron segmentos de transporte.")
        return

    st.subheader("Cadena de transporte configurada")
...
def panel_resultados(proyecto: Dict[str, str]) -> None:
    ...
    c1.download_button(
        ...
        data=csv_bytes,
        file_name="resultados_emisiones_multimodal.csv",
        mime="text/csv",
    )
    ...
    c2.download_button(
        ...
        file_name="trazabilidad_emisiones_multimodal.json",
    ...
    if MATPLOTLIB_DISPONIBLE:
        ...
        c3.download_button(
            ...
            data=pdf_bytes,
            file_name="reporte_emisiones_multimodal.pdf",
...
def panel_metodologia() -> None:
    with st.expander("Notas metodológicas de la herramienta"):
        st.markdown(
            ...
            **5. Multimodalidad**  
            La cadena total se construye como la suma de los segmentos carreteros, ferroviarios, marítimos, fluviales y aéreos efectivamente utilizados. Cada modo puede analizarse de forma independiente y posteriormente agregarse al resultado total.
